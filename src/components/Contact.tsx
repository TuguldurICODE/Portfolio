...
export default function Contact({ dark }: Props) {
  ...
  const links = [
    { label: 'Email', value: 'tuguldur@email.com', href: 'mailto:tuguldur@email.com', Icon: MailIcon },
    { label: 'GitHub', value: 'github.com/tuguldur', href: 'https://github.com/tuguldur', Icon: GithubIcon },
    { label: 'LinkedIn', value: 'linkedin.com/in/tuguldur', href: 'https://linkedin.com/in/tuguldur', Icon: LinkedinIcon },
  ]

  return (
    <section id="contact" style={{ padding: '56px 48px 100px' }}>
      ...
      <div style={{ maxWidth: 520 }}>
        ...
        <h2 style={{ fontSize: 32, fontWeight: 500, letterSpacing: '-1px', marginBottom: 12, color: t('#fff', '#111') }}>
          Let's work together
        </h2>
        <p style={{ fontSize: 14, lineHeight: 1.8, marginBottom: 40, color: t('#555', '#aaa') }}>
          Open to remote opportunities worldwide.<br />Feel free to reach out anytime.
        </p>
          ...
          {links.map(({ label, value, href, Icon }, i) => (
            <a
              ...
              style={{
                ...
                padding: '16px 0',
                borderBottom: t('1px solid rgba(255,255,255,0.06)', '1px solid rgba(0,0,0,0.07)'),
                textDecoration: 'none',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  ...
                  background: hovered === i ? t('#2a2a2a', '#e0e0de') : t('#1e1e1e', '#ebebea'),
                  border: t('1px solid rgba(255,255,255,0.06)', '1px solid rgba(0,0,0,0.06)'),
                  transition: 'background 0.2s',
                }}>
                  <Icon size={15} color={hovered === i ? t('#fff', '#111') : t('#666', '#888')} />
                </div>
                <span style={{ fontSize: 12, letterSpacing: '0.06em', textTransform: 'uppercase', color: t('#444', '#bbb') }}>{label}</span>
              </div>
              <span style={{ fontSize: 13, color: hovered === i ? t('#fff', '#111') : t('#555', '#aaa'), transition: 'color 0.2s' }}>{value}</span>
            </a>
          ))}
        </div>
      </div>
