...
export default function Projects({ dark }: Props) {
  ...
  return (
    <section id="projects" style={{ padding: isMobile ? '40px 24px' : '56px 48px' }}>
          ...
          <div key={p.title} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(null)}
            ...
            <div style={{ padding: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                {p.tags.map(tag => <span key={tag} style={{ fontSize: 11, padding: '3px 9px', borderRadius: 4, background: t('#222','#ebebea'), color: t('#666','#888'), border: t('1px solid rgba(255,255,255,0.05)','1px solid rgba(0,0,0,0.06)') }}>{tag}</span>)}
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                {p.github && <a href={p.github} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, padding: '5px 12px', borderRadius: 6, border: t('1px solid rgba(255,255,255,0.08)','1px solid rgba(0,0,0,0.08)'), color: t('#777','#888'), textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 5 }}>
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"/></svg>
                  GitHub
                </a>}
                {p.demo && <a href={p.demo} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, padding: '5px 12px', borderRadius: 6, background: t('#fff','#111'), color: t('#000','#fff'), textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 5, fontWeight: 500 }}>
